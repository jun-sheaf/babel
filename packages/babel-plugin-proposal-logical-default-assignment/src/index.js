  ...
  class DestructuringTransformer {
    ...
    push(id, _init, opRef) {
      ...
      if (t.isObjectPattern(id)) {
        ...
        this.pushAssignmentPattern(id, init, opRef);
      } else {
    ...
    pushAssignmentPattern({ operator, left, right }, valueRef, opRef) {
      ...
      operator = operator ?? opRef;
      let tempConditional;
      switch (operator) {
        case "||=":
        case "??=": {
          const operatorTrunc = operator.slice(0, -1);
          tempConditional = t.logicalExpression(
            operatorTrunc,
            t.cloneNode(tempId),
            right,
          );
        ...
        default: {
          tempConditional = t.conditionalExpression(
            t.binaryExpression(
              "===",
              t.cloneNode(tempId),
      ...
      if (t.isPattern(left)) {
        ...
        this.push(left, patternId, operator);
      } else {
  ...
  return {
    ...
    visitor: {
      AssignmentPattern(path) {
        const { node, scope, listKey } = path;
        const { operator, left, right } = node;
        switch (operator) {
          case "||=":
          case "??=": {
            const operatorTrunc = operator.slice(0, -1);
            if (listKey !== "params") return;
            const tempIdentifier = scope.generateUidIdentifier("darg");
            path.replaceWith(tempIdentifier);
            this.cache.push(
            ...
            let hasMore = false;
            path.getAllNextSiblings().forEach(sibling => {
              if (
                t.isAssignmentPattern(sibling.node) &&
                ["||=", "??="].includes(sibling.node.operator)
              ) {
                hasMore = true;
              }
            });
            if (!hasMore) {
              const func = path.getFunctionParent().get("body");
              this.cache.reverse().forEach(node => {
                func.unshiftContainer("body", t.cloneNode(node));
              });
              this.cache = [];
            }
          }
